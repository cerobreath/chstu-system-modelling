...
public class Model implements IStatisticsable, IExperimentable, ITransProcesable {
    ...
    public void initForTrans(double finishTime) {
        getExcavator().setFinishTime(finishTime);
        getLoader().setFinishTime(finishTime);
        getDumper().setFinishTime(finishTime);
        getCrusher().setFinishTime(finishTime);
    ...
    public Map<String, ITransMonitoring> getMonitoringObjects() {
        ...
        transMap.put("Черга до екскаватора", getQueueToExcavator());
        transMap.put("Черга до завантажувача", getQueueToLoader());
        transMap.put("Розмір купи руди", getOreHeap());
        transMap.put("Розмір бункера", getBunker());
        return transMap;
    }
}